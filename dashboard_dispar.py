...
st.caption("Dinas Pariwisata Kabupaten Pasuruan - Visualisasi & Pemetaan Usaha Masyarakat")
...
if uploaded_file is not None:
    try:
        ...
        # Chart for number of businesses per district
        if st.checkbox("📊 Grafik Jumlah Usaha Terbanyak per Kecamatan"):
        ...
        # ✅ Tambahan Fitur: Jenis Usaha Paling Dominan
        st.subheader(f"🏆 Jenis Usaha Paling Dominan di Kecamatan **{selected_kecamatan}**")
        ...
        if not dominan_df.empty:
            ...
            st.success(
                f"Jenis usaha paling dominan di **{selected_kecamatan}** adalah **{jenis_terbanyak['JENIS USAHA']}** "
                f"dengan jumlah **{jenis_terbanyak['JUMLAH']} usaha**."
            )

            with st.expander("📋 Lihat Rincian Jumlah per Jenis Usaha"):
                st.dataframe(dominan_df, use_container_width=True)
        else:
            st.warning("⚠️ Tidak ada data jenis usaha di kecamatan ini.")
