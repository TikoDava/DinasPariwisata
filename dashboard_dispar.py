...
st.markdown("""
    ...
    /* Selectbox and FileUploader labels */
...
# --- DATA EXTRACTION & LOADING FUNCTIONS ---
...
def load_data_from_drive(file_id, subsektor):
    """Loads a CSV file from Google Drive and preprocesses it."""
    ...
    # Read CSV, skipping initial rows (assuming metadata/headers)
    df = pd.read_csv(io.StringIO(response.content.decode('utf-8')), skiprows=3)

    current_cols = df.columns.tolist()
    
    # Define expected "Unnamed" columns and their desired new names
    ...
    if 'TENAGA KERJA TOTAL' not in required_cols and 'Unnamed: 6' in expected_unnamed_cols:
        required_cols.append('TENAGA KERJA TOTAL') # Add if it's supposed to be renamed

    # Filter columns to only include those that exist and are required
    df = df[[col for col in required_cols if col in df.columns]].copy()
    
    # Overwrite the 'SUBSEKTOR' column with the correct subsector from the dictionary
    # This is important as the spreadsheet itself might not have this column filled correctly
    df['SUBSEKTOR'] = subsektor 

    # Extract kecamatan from 'ALAMAT'
    if 'ALAMAT' in df.columns:
        df['KECAMATAN'] = df['ALAMAT'].apply(ekstrak_kecamatan)
    else:
        df['KECAMATAN'] = 'TIDAK DIKETAHUI' # Default if ALAMAT column is missing
...
# --- DATA CLEANING ---
# Remove rows where 'NAMA USAHA' or 'ALAMAT' are placeholder values or null
...
if df_all.empty:
    ...
    st.stop() # Stop if DataFrame is empty after cleaning
...
if filtered_df.empty:
    ...
    else: # Blok else ini sekarang hanya muncul jika kolomnya memang hilang, bukan karena data kosong
        st.info("ℹ️ Kolom 'JENIS USAHA' tidak ditemukan atau kosong. Fitur ini tidak dapat ditampilkan.")
    
    st.markdown("---")

    # ... (Bagian grafik lain tetap sama) ...
    ...
    with chart_col1:
        if selected_subsektor == '(Semua Subsektor)' and not filtered_df.empty:
            ...
            bars_subsektor = alt.Chart(subsektor_counts).mark_bar().encode(
                x=alt.X('Subsektor:N', sort='-y', title='Subsektor'),
                y=alt.Y('Jumlah Usaha:Q', title='Jumlah'),
                tooltip=['Subsektor', 'Jumlah Usaha'],
                color=alt.Color('Subsektor:N', legend=None)
            )
            text_subsektor = bars_subsektor.mark_text(
                align='center', 
                baseline='bottom', 
                dy=-5
            ).encode(
                text=alt.Text('Jumlah Usaha:Q'),
                color=alt.value('black')
            )
            subsektor_chart = (bars_subsektor + text_subsektor).properties(
                title=f"Distribusi Usaha per Subsektor di {selected_kecamatan}"
            ).interactive()
            st.altair_chart(subsektor_chart, use_container_width=True)
    ...
    with chart_col2:
        if selected_kecamatan == 'Semua Kecamatan' and not filtered_df.empty:
            ...
            bars_kecamatan = alt.Chart(kecamatan_counts).mark_bar().encode(
                x=alt.X('Kecamatan:N', sort='-y', title='Kecamatan'),
                y=alt.Y('Jumlah Usaha:Q', title='Jumlah'),
                tooltip=['Kecamatan', 'Jumlah Usaha'],
                color=alt.Color('Kecamatan:N', legend=None)
            )
            text_kecamatan = bars_kecamatan.mark_text(
                align='center',
                baseline='bottom',
                dy=-5
            ).encode(
            ...
            kecamatan_chart = (bars_kecamatan + text_kecamatan).properties(
                title=f"Distribusi Usaha per Kecamatan untuk Subsektor {selected_subsektor}"
            ).interactive()
            st.altair_chart(kecamatan_chart, use_container_width=True)
